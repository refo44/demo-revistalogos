...
- **Decisión de propietario 2026-08-19 (excepción, Option 2):** se permite un *editorial bootstrap* de Volume 1. El dataset Vol. 12 Nº 2 de la maqueta **sigue excluido como verdad editorial** y **nunca** se importa como `fixtures seed`. Bajo esta excepción, campos de presentación de esa maqueta (conteo, títulos, abstracts, secciones, orden, portada/PDF claramente placeholder) **pueden** transformarse en objetos Issue/Article de Vol. 1 Nº 1, editables, marcados `_les_bootstrap*`. **No** aplica a DOI/ORCID/ISSN falsos, paginación bibliográfica inventada ni autores dummy. Reutiliza el Author canónico `rafael-eduardo-figueredo-oropeza` (nunca lo crea, marca ni borra). Comando: `wp revistalogos fixtures bootstrap`. El seed demo permanece bloqueado en producción. La forma original de la excepción (un issue/article/author borradores con `_les_fixture_kind=bootstrap`) queda como historia.
- Indexación **considerada cerrada**. No abrir mientras queden fixtures demo públicas. Preferencia de lanzamiento: recuento `_les_fixture=1` = 0. Launch gate: `docs/operations/produccion-wordpress.md`.
