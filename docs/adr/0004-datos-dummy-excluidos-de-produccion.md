...
- **Decisión de propietario 2026-08-19 (excepción, actualizada en implementación):** se permite un *editorial bootstrap* de Volume 1 en producción — un `issue` y sus `article` como objetos editables (no el dataset demo), **sin** DOI/ORCID/ISSN falsos, reutilizando el Author canónico existente. Marcadores `_les_bootstrap*` (no `_les_fixture` disposable). Comando: `wp revistalogos fixtures bootstrap`. El seed demo (`wp revistalogos fixtures seed`) **sigue bloqueado** en producción salvo `--allow-production`, que **no** se usa en el live. La forma original de la excepción (un issue/article/author borradores con `_les_fixture_kind=bootstrap`) queda como historia; el código vigente es Volume 1 + adopción.
