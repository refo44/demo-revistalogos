...
| D18 | Cómo Citar: apellido bibliográfico hispano (ambos) en **todos** los formatos según cada estándar; un apellido = heurística actual; campo opcional `citation_surname` en el autor; override opcional por formato (7 cajas) en el artículo; Regenerar uno por uno borra ese override. **No** heurística de 4 tokens. Highwire/PDF/Fase 4 fuera | Implementación: [trabajo pendiente](#trabajo-pendiente-aceptado) ítem 11 | ✅ Resuelta ([0021](0021-apellido-bibliografico-y-override-como-citar.md)); WU en `main` ([#64](https://github.com/refo44/demo-revistalogos/issues/64) cerrado). §2 sustituida por [0022](0022-nombres-y-apellidos-de-autor.md) |
| D19 | Autor: **Nombres** (`given_names`) y **Apellidos** (`family_names`) obligatorios en ficha nueva (≥1 token cada uno); **`citation_surname` opcional** (relleno = cita; vacío = `family_names`); título público intacto; backfill por **ajuste temporal** (snapshot → Apply; Restore; se retira en N+1). **No** `maybe_upgrade`. **No** cuatro slots. **No** componer el título. | Implementación: [trabajo pendiente](#trabajo-pendiente-aceptado) ítem 12 | ✅ Resuelta ([0022](0022-nombres-y-apellidos-de-autor.md)); WU en `main` (`v0.3.15`, [#67](https://github.com/refo44/demo-revistalogos/issues/67) cerrado). El próximo deploy retira el puente |
...
**Estado:** HECHO en `main` (`v0.3.14`). ADR **aceptado** ([0021](0021-apellido-bibliografico-y-override-como-citar.md)). Issue: [#64](https://github.com/refo44/demo-revistalogos/issues/64) cerrado. Independiente del spike de colapso (ítem 9 / [#15](https://github.com/refo44/demo-revistalogos/issues/15)). §2 sustituida por el ítem 12.
...
3. Theme: el builder usa `citation_surname` cuando hay valor; si un override está relleno, muestra ese texto. Un apellido / vacío sigue As-Is.
4. Regenerar **por formato**: borra ese override; no copia texto generado al meta.
5. Título, DOI, páginas y slug se editan en sus campos, no parcheando la cita.
...
Plugin 0.2.19 / theme 0.2.11 en `main` ([#64](https://github.com/refo44/demo-revistalogos/issues/64) cerrado). §2 sustituida por el ítem 12 / ADR 0022.

#### 12. Autor — Nombres y Apellidos almacenados + backfill reversible

**Estado:** HECHO en `main` (`v0.3.15`). ADR **aceptado** ([0022](0022-nombres-y-apellidos-de-autor.md)). Issue: [#67](https://github.com/refo44/demo-revistalogos/issues/67) cerrado. Independiente del spike de colapso (ítem 9) y de los overrides de 0021 (ítem 11, hecho). Plugin 0.2.20 / theme 0.2.12. El **siguiente** deploy etiquetado retira el ajuste Apply/Restore.
...
Backfill: **ajuste temporal** wp-admin (no `maybe_upgrade`). Snapshot → Apply (`family_names` desde `citation_surname` o last-token; Nombres = resto; no pisar relleno; **no escribir** `citation_surname`) → Restore. El **siguiente** deploy retira el ajuste (puente, como `Bootstrap_Admin`). Sin `fixtures seed`. TDD. Gutenberg playbook [#30](https://github.com/refo44/demo-revistalogos/issues/30). Metabox en columna principal. Sin deploy implícito (ADR 0020).

### DEFERRED

#### 5. PDF de número / número completo

**Estado:** DEFERRED. **No iniciado.** Issue: [#33](https://github.com/refo44/demo-revistalogos/issues/33). [ADR 0017](0017-generacion-automatica-pdf-articulo.md) §7 (fuera de v1) y D17. Exige **ADR propio** (no reabrir 0017). **No** mezclar con WU7 (ítem 4). WU7 va antes: Generar/Regenerar de artículo consume la plantilla de separata. El compositor del número lee `issue` + artículos publicados + `pdf_file` del issue **sin** cambiar ese contrato (ADR 0017 §7). Dominio en `revistalogos-core` (ADR 0005). Theme/FSE solo **consumen** `pdf_file`; no generan. Sin backfill al activar o actualizar. No implementar ahora.
