...
| D17 | Generación automática del PDF de artículo al publicar → **arquitectura aceptada** ([0017](0017-generacion-automatica-pdf-articulo.md)): `pdf_file` sigue siendo un ID de Media Library; publicar exigirá PDF válido; si falta, generar; si falla, bloquear; no pisar un PDF válido; no regenerar al guardar; no backfill en upgrade. **Testing Foundation hecha** ([0018](0018-testing-foundation.md), `docs/23`). **WU1–WU4** política, adaptador, orquestación y renderer Dompdf (Composer del plugin). PDF de número: fuera de v1. Hoy el PDF sigue siendo opcional y manual. | Persistencia Media Library + cableado de publicación (TDD); no activar la regla hasta entonces | ✅ Resuelta ([0017](0017-generacion-automatica-pdf-articulo.md)); implementación iniciada (WU1–WU4) |
