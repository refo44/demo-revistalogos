...
**Testing Foundation: IMPLEMENTADA** (ADR 0018, `docs/23-testing-foundation.md`, PHPUnit 9.6, `composer test:unit`). ADR 0017 permanece **aceptado**. WU1 (política de dominio pura), WU2 (adaptador WordPress de solo lectura) y WU3 (orquestación + seam de renderer, sin librería real) están en `revistalogos-core`. **No** hay renderer real, **no** hay creación de adjuntos, **no** hay orquestación de publicación en WordPress. Publicar **sigue** sin exigir PDF. El PDF integral del número sigue siendo carga editorial manual. Ver ADR 0017.
