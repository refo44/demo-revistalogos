# {{PROJECT_NAME}} — Layout Principles
...
**Reference:** `17-implementation-order`, `18-ux-ui-trends`

---

## 1. Reading width

- **Goal:** 60–70 characters per line (`ch`) in body text.
- **Implementation:** Content container with `max-width` in `ch` (e.g. `65ch`).
...
- **Consistency:** Margins and padding follow a scale (e.g. multiples of 1rem).
...
- **Rule:** Single spacing system in main.css; no arbitrary values per page.
...
- **Proportion:** When text and image share a block, define clear proportion (50/50, 2/3–1/3) per wireframes.
...
- **Center:** Content in a centered, aligned container.
...
- **Interactives:** Easy to tap; sufficient space around.
...
| Vertical rhythm | Single scale; generous; consistent across pages |
