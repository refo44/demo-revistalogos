...
| Despliegue WP rechaza ref de rama aunque HEAD esté etiquetado (ADR 0020) | `./tools/qa-require-production-release-tag.sh` | exit 1 con `GITHUB_REF=refs/heads/main` sobre commit etiquetado; exit 0 con `refs/tags/v1.2.3` y sin `GITHUB_REF` (preflight local). Job aborta antes de FTPS. | Pass (working tree) | working tree 2026-09-10 |
...
| Issue #30 Gutenberg REST + metabox meta | PHPUnit WordPress `ArticleAuthorPublicationRestTest` (`composer test:wp`) + `tools/qa-article-editorial-ux.sh` + Gherkin `article-author-publication.feature` | CPT `custom-fields`; esquema REST con authors/issue/pdf_file; publish sin autor → 400; same-request con authors+issue+pdf_file+title_en persiste; JS `syncMetaboxMetaToBlockEditor` / `collectMetaboxMeta`; HTTP same-session 200 | Pass (working tree) | working tree 2026-08-31 |
...
| Registro de CPTs/taxonomías con slugs ADR 0008 | activación + resolución de `/revista/numeros`, `/revista/articulos`, `/revista/autores/` y términos de fixtures | archivos y singles de issue/article 200 (dfb91b8). Single CPT `author`: 404 histórico 2026-08-18; `query_var=journal_author` corrige el single localmente (ver `tools/qa-author-permalinks.sh`) | Pass (local) | working tree 2026-08-19 |
...
| CF7 en página de contacto (ADR 0010) | `tools/qa-contact-form.sh`: CF7 desde WordPress.org + `maybe_provision` | formulario CF7 renderizado en `/contacto/` (marcadores `wpcf7` presentes); envío de correo no probado (sin SMTP local) | Pass (local) | working tree 2026-09-14 |
| CF7 en producción (ADR 0010, issue #62) | `curl` a `https://logo-et-spes.cenfiss.net/contacto/`; confirmación del propietario de que el formulario envía | CF7 **6.1.7**, formulario id **202**, campos Nombre completo/Email/Asunto/Mensaje obligatorios, botón «Enviar Mensaje», honeypot `les_website_url`, enlace a `/privacidad/`; sin texto fallback «Puede escribirnos»; 0 `Set-Cookie` en GET anónimo; Flamingo `404`; sin reCAPTCHA. Envío a `revista.cenfiss@gmail.com` confirmado por el propietario. | Pass (working tree) | working tree 2026-09-17 |
...
| page-contacto.html | page-contacto.php | content-institutional-page + región CF7 | page content + CF7 (`revistalogos_contact_form_id`) | Implemented | Unverified (paridad visual) | CF7 6.1.7 live en producción (formulario id 202, 2026-09-17); el `mailto:` institucional del aside no es el fallback del formulario (ADR 0010) |
