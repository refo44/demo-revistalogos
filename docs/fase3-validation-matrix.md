...
| ADR 0017 WU6A source + composición explícita | `tools/qa-article-pdf-composition.sh` Docker aislado puerto 8088 | HTML local título/cuerpo/autores; Gutenberg sin comentarios de bloque; generate → adjunto `%PDF-` + `pdf_file` ID; KEEP_EXISTING no reemplaza; artículo draft; publish sin generate no crea PDF; guard de autor intacto | Pass (working tree) | working tree 2026-08-23 |
