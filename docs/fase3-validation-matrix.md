...
| Issue #30 Gutenberg publish + autor en la misma petición | PHPUnit WordPress `ArticleAuthorPublicationRestTest` (`composer test:wp` / `tools/run-phpunit-wp.sh`) + `tools/qa-article-editorial-ux.sh` + Gherkin `article-author-publication.feature` | CPT `article` soporta `custom-fields`; esquema REST expone `meta.authors`; publish REST de borrador sin autor → 400 y sigue draft; publish REST de borrador con `meta.authors` en la misma petición → publish + autores persistidos; HTTP same-session 200 | Pass (working tree) | working tree 2026-08-31 |
